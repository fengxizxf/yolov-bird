...
def detect(opt, save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            st.write(f'{s}Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS')
            # Stream results
            if view_img:
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        # streamlit中的video适合播放h264编码的mp4，将'mp4v'改成了'avc1'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='best100.pt',
                        help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='100birds/video/', help='source')  # file/folder, 0 for webcam
